...
export const getByA = createAsyncThunk(
    "postulations/getByA",
    async(data, thunkAPI) => {
        try {
            const token = thunkAPI.getState().auth.user.token;
            return await service.getByA(data, token);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const accept = createAsyncThunk(
    ...
    async(id, thunkAPI) => {
        try {
            return await service.accept(id);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const getByRec = createAsyncThunk(
    "postulations/getByRec",
    async(id, thunkAPI) => {
        try {
            return await service.getByRec(id);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const PostulationSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getByA.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(getByA.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.postulations = action.payload;
            })
            .addCase(getByA.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.message = action.payload;
            })
            ...
            .addCase(accept.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                console.log(state.postulations, "before");

                state.postulations[
                    state.postulations.findIndex(
            ...
            .addCase(getByRec.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(getByRec.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.postulations = action.payload;
            })
            .addCase(getByRec.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.message = action.payload;
            })
