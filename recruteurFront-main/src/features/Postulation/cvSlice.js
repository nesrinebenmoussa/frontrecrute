...
// get condidat by id
export const getbyDiplome = createAsyncThunk(
    "cv/getbydiplome",
    async(id, thunkAPI) => {
        try {
            const token = thunkAPI.getState().auth.user.token;
            return await CVservice.getbyDiplome(id, token);
        } catch (error) {
            const message =
                (error.response &&
                    error.response.data &&
...
export const CVslice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(getbyDiplome.pending, (state) => {
                state.isLoading = true;
            })
            .addCase(getbyDiplome.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.cv = action.payload;
            })
            .addCase(getbyDiplome.rejected, (state, action) => {
                state.isLoading = false;
                state.isError = true;
                state.message = action.payload;
            })
